...
import { BUNDLE_DESCRIPTION_MAX_LENGTH, BUNDLE_LABEL_MAP, MAX_BUNDLE_COUNT } from '../constants';
                  ...
                  <div className="mb-6 p-7 bg-neutral-100" key={field.id}>
                    ...
                    <div className="d-flex gap-6">
                      ...
                      <div className="mb-3 w-50">
                        <label className="form-label text-neutral-700 fs-7" htmlFor={getFieldId('bundle-title')}>
                          {bundlePrefix}名稱
                        </label>
                        <input
                          id={getFieldId('bundle-title')}
                          className={clsx('form-control', errors.bundle?.[index]?.title && 'is-invalid')}
                          type="text"
                          placeholder={bundlePrefix && `請輸入${bundlePrefix}名稱`}
                          {...register(`bundle.${index}.title`)}
                        />
                        {errors.bundle?.[index]?.title && (
                          <div className="invalid-feedback">{errors.bundle[index].title?.message}</div>
                        )}
                      </div>
                      {/* 內容物英文名 */}
                      <div className="mb-3 w-50">
                        <label className="form-label text-neutral-700 fs-7" htmlFor={getFieldId('bundle-en-name')}>
                          {bundlePrefix}英文名
                        </label>
                        <input
                          id={getFieldId('bundle-en-name')}
                          className={clsx('form-control', errors.bundle?.[index]?.enName && 'is-invalid')}
                          type="text"
                          placeholder={bundlePrefix && `請輸入${bundlePrefix}英文名`}
                          {...register(`bundle.${index}.enName`)}
                        />
                        {errors.bundle?.[index]?.enName && (
                          <div className="invalid-feedback">{errors.bundle[index].enName.message}</div>
                        )}
                      </div>
                    </div>
                    {/* 內容物描述 */}
                    <div>
                      <label className="form-label text-neutral-700 fs-7" htmlFor={getFieldId('bundle-description')}>
                        {bundlePrefix}描述
                      </label>
                      <textarea
                        id={getFieldId('bundle-description')}
                        className={clsx(
                          'form-control min-h-14rem',
                          errors.bundle?.[index]?.description && 'is-invalid',
                        )}
                      ...
                      />
                      {errors.bundle?.[index]?.description && (
                        <div className="invalid-feedback">{errors.bundle[index].description.message}</div>
                      )}
                    </div>
                  </div>
                ))}
