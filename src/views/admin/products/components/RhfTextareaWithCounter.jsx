...
function RhfTextareaWithCounter(
  ...
  const showCounter = typeof inputValue === 'string' && inputValue.length >= 0;
    ...
    <>
      <label className="form-label text-neutral-700 fs-7 d-flex" htmlFor={id}>
        ...
        <span className="fs-xs ms-auto text-neutral-500 align-self-end">
          {showCounter && `（${inputValue.length} / ${maxTextLength} 字）`}
        </span>
      </label>
      <textarea
        id={id}
        ...
        className={clsx('form-control', showError && 'is-invalid is-invalid--no-image', className)}
        {...props}
      />
...
export default forwardRef(RhfTextareaWithCounter);
